...
#define CAMERA_MOVE_SPEED	(10.0f)	// カメラの移動速度
...
{	
	// グローバル変数の初期化
	{
		m_dataPosV.Set(D3DXVECTOR3(-1300.0f, 2000.0f, -1500.0f));	// 視点
		m_dataPosVDest.Set(m_dataPosV.Get());						// 目的の視点

		m_dataPosR.Set(D3DXVECTOR3(0.0f, 0.0f, 0.0f));			// 注視点
...
void CCamera::Debug(void)
{
	...
	// 位置
	pDebugProc->Print("-----カメラの情報-----");
	pDebugProc->Print("\n");
	pDebugProc->Print("視点::POS_X[%f],POS_Y[%f],POS_Z[%f]", posV.x,posV.y,posV.z);
	pDebugProc->Print("\n");
	pDebugProc->Print("注視点::POS_X[%f],POS_Y[%f],POS_Z[%f]", posR.x,posR.y,posR.z);
	pDebugProc->Print("\n\n");
}
