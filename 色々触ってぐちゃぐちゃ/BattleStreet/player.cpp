...
#define PLAYER_SPEED		(2.5f)	// プレイヤーの速度
...
HRESULT CPlayer::Init(D3DXVECTOR3 pos , D3DXVECTOR3 rot,CModel::MODEL_TYPE modelType, CMotion::MOTION_TYPE motionType, int nStateMax)
{
	...
	{
		// モーションの生成
		m_pMotion = CMotion::Create(m_nNumModel, motionType);
...
void CPlayer::UpdateMotionNone(void)
{
	...
	if (pMotion->GetStateNext() == MOTION_STATE_PUNCH && m_data.motionState != MOTION_STATE_PUNCH ||
		pMotion->GetStateNext() == MOTION_STATE_KICK && m_data.motionState != MOTION_STATE_KICK ||
		pMotion->GetStateNext() == MOTION_STATE_PUNCH_FINISH && m_data.motionState != MOTION_STATE_PUNCH_FINISH ||
		pMotion->GetStateNext() == MOTION_STATE_KICK_FINISH && m_data.motionState != MOTION_STATE_KICK_FINISH)
	{
		if (m_pAttack != nullptr)
		{
			// 終了処理
	...
	if (pMotion->GetStateNext() == MOTION_STATE_DAMAGE && m_data.motionState != MOTION_STATE_DAMAGE ||
		pMotion->GetStateNext() == MOTION_STATE_DAMAGE_BIG && m_data.motionState != MOTION_STATE_DAMAGE_BIG )
	{
		m_data.state = STATE_NEUTRAL;
	}

	if (m_data.motionState != pMotion->GetStateNext())
	{
		pMotion->Set(m_data.motionState);
	}

	// モーションの終了状況を判定
	if (pMotion->GetIsFinsih())
	{
		// 待機状態を設定
		m_data.motionState = MOTION_STATE_NEUTRAL;
	}
