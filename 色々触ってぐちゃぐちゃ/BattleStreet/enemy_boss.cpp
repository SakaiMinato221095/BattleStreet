...
void CEnemyBoss::UpdateMotion(void)
{
	...
	if (pMotion->GetStateNext() == MOTION_STATE_DAMAGE && m_infoVisual.motionState != MOTION_STATE_DAMAGE ||
		pMotion->GetStateNext() == MOTION_STATE_BIG_DAMAGE && m_infoVisual.motionState != MOTION_STATE_BIG_DAMAGE)
	{
	...
	// モーション状態と現在のモーションを比較
	if (pMotion->GetStateNext() != m_infoVisual.motionState)
	{
		// モーション情報を設定
