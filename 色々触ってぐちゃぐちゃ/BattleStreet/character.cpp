...
//- プレイヤーの初期化処理
//-------------------------------------
HRESULT CCharacter::Init(CModel::MODEL_TYPE modelType, CMotion::MOTION_TYPE motionType)
{
	// モデルのパーツ数を取得
...
void CCharacter::Update(void)
{
	...
	if (m_pMotion != NULL)
	{
		if (m_pMotion->GetIsFinsih() == false)
		{
			// モーションの更新処理
			m_pMotion->Update();
		}
...
//- プレイヤーの生成処理
//-------------------------------------
CCharacter* CCharacter::Create(CModel::MODEL_TYPE modelType, CMotion::MOTION_TYPE motionType)
{
	// プレイヤーのポインタを宣言
	...
	{
		// 初期化処理
		if (FAILED(pPlayer->Init(modelType, motionType)))
		{// 失敗時
